...
# Execute GSEA
do_not_touch/GSEA_4.3.3/gsea-cli.sh GSEAPreranked \
  -gmx "${snakemake_input[gmt]}" \
  -collapse No_Collapse \
  -norm meandiv \
  -nperm 1000 \
  -rnd_seed 149 \
  -rnk "${snakemake_input[rank]}" \
  -scoring_scheme weighted \
  -rpt_label "${snakemake_params[rpt_label]}" \
  -create_svgs false \
  ...
  -set_min 5 \
  -zip_report false \
  -out "${snakemake_params[out_dir_gsea]}"
